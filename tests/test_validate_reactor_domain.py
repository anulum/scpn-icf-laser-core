...
# SCPN ICF Laser Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"configurations": ["laser_icf_direct_drive", "laser_icf_direct_drive"]},
            "must be unique",
        ),
        (
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-ICF-LASER-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-ICF-LASER-CORE"
                },
