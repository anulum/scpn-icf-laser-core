...
fusion (ICF) systems within the SCPN Reactor Systems Research Group. This
repository is the designated owner of device-level truth for the three
laser-ICF configurations of the SCPN Phase Orchestrator reactor registry:
`laser_icf_direct_drive` (spherical direct-drive target),
`laser_icf_indirect_drive` (hohlraum indirect-drive target), and
...
`VALIDATION.md#diagnostic-and-clock-semantics`); level-0 device
...
`VALIDATION.md#level-0-device-physics`); the device 3D model — the
tessellated capsule, its layers and the radiation enclosure of an
indirect-drive target, built on the shared reactor kernel library
(ADR 0006, evidence: `VALIDATION.md#device-3d-model`); and the device
CAD model — the same bodies as exact solids with fail-closed evidence
against their analytic forms and a normalised STEP export, behind the
optional `cad` extra (ADR 0006, evidence:
...
This repository owns, for the laser-ICF device family:

- the device boundary: plant and experiment truth, shot lifecycle, and
  configuration policy for capsule implosions driven by high-energy laser
  systems — direct illumination of the capsule, indirect X-ray drive from
  a laser-heated hohlraum, and staged schemes that separate compression
  from ignition (fast ignition's short-pulse igniter, shock ignition's
  late strong shock);
- drive-campaign semantics at device level: pulse-shaping declarations,
  beam-geometry and symmetry-control declarations, target and capsule
  metrology contracts, and shot-cycle logistics;
...
- **Ion-beam and pulsed-electron-beam ICF**: `SCPN-ICF-BEAM-CORE`.
- **Projectile and impact ICF**: `SCPN-ICF-IMPACT-CORE`.
- **Magnetised liner inertial fusion** (MagLIF): `SCPN-MIF-MAGLIF-CORE`.
- **Solver mathematics and validation evidence** (implosion
  hydrodynamics, radiation transport, burn physics): `SCPN-FUSION-CORE`
  until an exact surface passes the reactor family migration gate; no
  solver code exists in, or was copied into, this repository.
...
artefact, and no parameter set describes or validates any real machine. Drive-scheme, target-design, and fuel choices are
