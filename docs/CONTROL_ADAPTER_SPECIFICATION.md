...
SCPN ICF Laser Core — CONTROL adapter specification
-->

# CONTROL adapter specification

**Adapter identifier:** `scpn-icf-laser-core.control-adapter`  
**Contract version:** `0.1.0-spec` (specification only — **no implementation
exists**)  
**Consumer:** `SCPN-CONTROL` plugin protocol

This document is the device-owned contract through which a future laser-ICF
...
- channel identifier and physical quantity with SI unit — including drive
  channels (delivered energy and power balance per beam or cone,
  backscatter fractions, pulse-shape conformance) and implosion/burn
  channels (implosion velocity and shape proxies, neutron yield, bang
  time as the declared timing anchor, ion-temperature proxies);
- coordinate convention (target-chamber frame, capsule-relative, or
  hohlraum-relative label per declared drive scheme) and spatial layout
  (scalar, per-beam array, or image-derived);
- sampling clock identity and timestamp semantics (shot-relative time base
  with picosecond-class resolution declared per channel);
...
- shot-cycle lifecycle phase (`target_acceptance`, `insertion_alignment`,
  `laser_charge`, `shot`, `burn_window`, `post_shot_recovery`,
  `terminated`) with monotonic transition semantics and device-truth
  event records for pulse-shape non-conformance and alignment faults;
- configuration identity: which registry configuration
  (`laser_icf_direct_drive`, `laser_icf_indirect_drive`, or
  `laser_icf_fast_or_shock_ignition`), which target/campaign design
  revision, and which configuration policy revision produced the shot;
...
For each actuator class (pulse-shape and power-balance programming, beam
pointing and smoothing configuration, target insertion and alignment
systems, cryogenic layering control where declared, short-pulse igniter
timing for the staged scheme):
...
- machine-readable operational envelope (delivered-energy and fluence
  bounds against optics damage, repetition limits, target-handling and
  cryogenic bounds with applicability domains);
- declared device-level hazard semantics for optics damage, misalignment,
  and pulse-shape non-conformance;
...
above through CONTROL's real admission path: nominal shot cycles, degraded
symmetry and non-conformance sequences, boundary-of-envelope cases,
